...
def calculate_multi_mape(model1, model2, x_train, z_train_group, stolen_num):
    ...
    for index in range(stolen_num):
        ...
        data = 0.07 * data
        mape += np.mean(np.abs(stolen_data - data)/0.07)*255
...
def show_multi_data(model1,model2,x_train, z_train_group,stolen_num):
    ...
    for z_train in z_train_group:
        ...
        data = stolen_data/0.07
        print(data)
        data_group.append(data)
    return data_group


def multi_cifar10_vgg13_linear_noattack_train(conv_net, fc_net, optimizer, gama, total_epoch, stolen_num):
    ...
    # 将训练集图片转成tensor并展平
    x_train_process = tf.convert_to_tensor(x_train.flatten(), dtype=tf.float32) / 255
    ...
    test_accuracy_name = 'multi_cifar10_result//multi_cifar10_rmattack_result' + '_test_accuracy_' + str(total_epoch)  + '_' + str(stolen_num)
    mape_name = 'multi_cifar10_result//multi_cifar10_rmattack_result' + '_mape_' + str(total_epoch) + '_' + str(stolen_num)
    data_name = 'multi_cifar10_result//multi_cifar10_rmattack_result' + '_stolen_data_' + str(total_epoch) + '_' + str(stolen_num)
    loss_name = 'multi_cifar10_result//multi_cifar10_rmattack_result' + '_loss_' + str(total_epoch) + '_' + str(stolen_num)
